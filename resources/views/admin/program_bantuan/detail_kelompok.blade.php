    ...
    <table width="100%" class="table table-secondary" style="table-layout: fixed">
        ...
        <thead>
            <tr>
                ...
                <th rowspan="2">Ketua Kelompok</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    <a onclick="return confirm('Hapus penerima {{ $penerima->penerimaKelompok->nama }}?')" class="btn btn-danger btn-aksi" href="/admin/program-bantuan/penerima/hapus?penerima={{ $penerima->id }}"><i class="fas fa-times"></i></a>
                </td>
                <td>{{ $penerima->penerimaKelompok->nama }}</td>
                <td>{{ $penerima->penerimaKelompok->ketua->nama }}</td>
                <td>{{ $penerima->id }}</td>
                <td>{{ $penerima->penerimaKelompok->ketua->nik }}</td>
                <td>{{ $penerima->penerimaKelompok->ketua->nama }}</td>
                <td>{{ $penerima->penerimaKelompok->ketua->ttl }}</td>
                <td>{{ $penerima->penerimaKelompok->ketua->jenis_kelamin }}</td>
                <td>{{ $penerima->penerimaKelompok->ketua->alamat }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
...
<div class="modal fade" id="tambah-data" tabindex="-1" aria-labelledby="tambah-data-label" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="/admin/program-bantuan/penerima/tambah?bantuan={{ $bantuan->id }}&fkey=kode_kelompok_penerima&desa={{ $desa->id }}" method="post">
      @csrf
        <div class="modal-body">
            <div class="mb-3">
                <label for="fkey_value" class="form-label">Nama/Kode Kelompok Penerima</label>
