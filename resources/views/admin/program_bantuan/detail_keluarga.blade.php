        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    <a onclick="return confirm('Hapus penerima {{ $penerima->penerimaKeluarga->nama }}?')" class="btn btn-danger btn-aksi" href="/admin/program-bantuan/penerima/hapus?penerima={{ $penerima->id }}"><i class="fas fa-times"></i></a>
                </td>
                <td>{{ $penerima->penerimaKeluarga->Nomor_KK }}</td>
                <td>{{ $penerima->penerimaKeluarga->NIK }}</td>
                <td>{{ $penerima->penerimaKeluarga->kepala->nama }}</td>
                <td>{{ $penerima->id }}</td>
                <td>{{ $penerima->penerimaKeluarga->kepala->nik }}</td>
                <td>{{ $penerima->penerimaKeluarga->kepala->nama }}</td>
                <td>{{ $penerima->penerimaKeluarga->kepala->ttl }}</td>
                <td>{{ $penerima->penerimaKeluarga->kepala->jenis_kelamin }}</td>
                <td>{{ $penerima->penerimaKeluarga->kepala->alamat }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
...
<div class="modal fade" id="tambah-data" tabindex="-1" aria-labelledby="tambah-data-label" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="/admin/program-bantuan/penerima/tambah?bantuan={{ $bantuan->id }}&fkey=no_kk_penerima&desa={{ $desa->id }}" method="post">
      @csrf
            ...
            <div class="mb-3">
                ...
                <div class="suggested-input">
                    ...
                    <ul class="suggestion" id="suggestion">
                        @foreach ($keluarga as $kelg)
                        <li>{{ $kelg->kepala->nama }} - {{ $kelg->Nomor_KK }}</li>
                        @endforeach
                    </ul>
                </div>
            </div>
